...
extension AppModel {
    func checkLocal() async {
        ...
        do {
            ...
            diagnostics = result["summary"] as? String ?? L10n.text("ローカル環境を確認しました。")
            detail = diagnostics
    ...
    func testSpeech() {
        ...
        launch { [self] run in
            guard !speechTestText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty,
                  speechTestText.count <= 20000 else { throw RelayError.message(L10n.text("試聴する文章は1〜20,000文字で入力してください。")) }
            ...
            guard run == epoch else { return }
            phase = .stopped
            detail = L10n.text("読み上げが終わりました。発音と声質を確認してください。")
        }
    }
    ...
    func testMicrophone() {
        ...
        stop()
        phase = .preparing
        detail = L10n.text("許可済みのマイクをテストします。Webexへは送信しません。")
        launch { [self] run in
            ...
            recorder.onConfigurationChange = { [weak self] in Task { @MainActor in
                ...
                self.fail(RelayError.message(L10n.text("テスト中にマイクの音声形式が変わりました。入力デバイスが安定してから、もう一度テストしてください。")))
            } }
            ...
            phase = .recording
            detail = L10n.text("5秒間のテスト録音中です。合言葉と短い指示を話してください。送信はしません。")
            ...
            guard !samples.isEmpty else {
                throw RelayError.message(L10n.text("マイクから音声フレームが届きませんでした。macOSのサウンド設定と会議アプリの入力デバイスを確認し、待受を停止してから再テストしてください。"))
            }
            guard peak > 0 else {
                throw RelayError.message(L10n.text("音声フレームは届いていますが、入力音量がゼロです。macOSのサウンド設定で内蔵マイクの入力音量を確認してください。"))
            }
            try AudioRecorder.write(samples, to: file)
            phase = .recognizing
            audioStatus = String(format: L10n.text("5秒録音の入力レベル最大: %.0f%%（待受と同じ録音経路）"), peak * 100)
            ...
            phase = .stopped
            detail = recognizedInput.isEmpty ? (result["rejected"] as? String ?? L10n.text("認識できませんでした。入力デバイスと音量を確認してください。")) :
                (match ? L10n.text("マイク・文字起こし・合言葉の一致を確認しました。送信していません。") : L10n.text("文字起こしは成功しましたが、合言葉が一致しません。表示された表記を別候補として登録できます。"))
        }
    }
