...
final class SpeakerDiagnosticsTests {
    ...
    @Test func absentSimilarityReturnsOnlyTheNote() {
        ...
        #expect(bare.summary(mode: "strict", threshold: 0.5) == "")
        #expect(bare.summary(mode: "prefer", threshold: 0.5) == "")
        let noted = SpeakerDiagnostics(result: ["speaker_note": "登録音声がありません", "overall_similarity": 0.9, "speaker_windows": [window(1, 0.5)]])
        #expect(noted.summary(mode: "strict", threshold: 0.5) == "登録音声がありません")
        #expect(noted.summary(mode: "prefer", threshold: 0.5) == "登録音声がありません")
    }
    @Test func preferModeAppendsAReferenceLineToTheNote() {
        let reference = String(format: "\n話者類似度 %.3f（参考値・単独発話を拒否するしきい値ではありません）", 0.4321)
        #expect(SpeakerDiagnostics(result: ["similarity": 0.4321]).summary(mode: "prefer", threshold: 0.5) == reference)
        #expect(SpeakerDiagnostics(result: ["similarity": 0.4321]).summary(mode: "prefer", threshold: 0.5).hasPrefix("\n"))
        ...
        #expect(noted.summary(mode: "prefer", threshold: 0.5) == "注記" + reference)
    }
    @Test func strictModeIncludesOverallAndWindows() {
        ...
        #expect(parsed.summary(mode: "strict", threshold: 0.5) == expected)
    }
    @Test func strictModeOmitsOverallWhenAbsent() {
        let text = SpeakerDiagnostics(result: ["similarity": 0.61, "speaker_windows": [window(1.5, 0.55)]]).summary(mode: "strict", threshold: 0.5)
    ...
    @Test func windowSuffixDistinguishesAbsentEmptyAndMixedEntries() {
        ...
        #expect(absent.summary(mode: "strict", threshold: 0.5) == strict(0.61, 0.5))
        ...
        #expect(empty.summary(mode: "strict", threshold: 0.5) == strict(0.61, 0.5) + "\n区間別: ")
        ...
        #expect(mixed.summary(mode: "strict", threshold: 0.5) == strict(0.61, 0.5) + "\n区間別: " +
    ...
    @Test func anyModeOtherThanPreferUsesTheStrictText() {
        ...
        for mode in ["strict", "", "PREFER", "prefer ", "unknown"] {
            #expect(parsed.summary(mode: mode, threshold: 0.25) == strict(0.61, 0.25))
        }
    }
    @Test func nonNumericOverallIsIgnored() {
        ...
        #expect(parsed.summary(mode: "strict", threshold: 0.5) == strict(0.61, 0.5))
    }
}
