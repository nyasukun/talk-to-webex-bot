    ...
    var body: some View {
        Form {
            Group {
                ...
                } header: { Text("取得済みのローカル環境") } footer: { Text("結果はログ画面で確認できます。モデルが未配置の場合は案内を表示し、自動で取得しません。Qwenの標準は models/voice-1.7b（1.7B・8bit）です。以前の models/voice（0.6B・4bit）は省メモリ用に残せます。") }
                Section {
