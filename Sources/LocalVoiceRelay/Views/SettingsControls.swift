...
struct ReferenceSettings: View {
    ...
    var body: some View {
        LabeledContent(L10n.text("参照音声")) { Label(registered ? L10n.text("登録済み") : L10n.text("未登録"), systemImage: registered ? "checkmark.circle.fill" : "waveform").foregroundStyle(registered ? Color.teal : Color.secondary) }
        HStack {
            if model.referenceRecording {
                Label(L10n.text("録音中です。普段の声で話してください。"), systemImage: "record.circle").foregroundStyle(.red)
                Spacer()
                Button(L10n.text("録音終了")) { model.finishReference() }.tint(.red)
            } else {
                Button(L10n.text("参照音声を録音"), systemImage: "mic") { Task { await model.startReference(kind: kind) } }
                    .disabled(!model.canConfigure || model.permissionSnapshot.microphone != .allowed)
                Button(L10n.text("音声ファイルを選ぶ"), systemImage: "folder") { model.chooseReference(kind: kind) }.disabled(!model.canConfigure)
            }
        }
    }
}
