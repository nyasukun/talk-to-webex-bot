...
extension AppModel {
    ...
    func authenticationSucceeded(dismissRenewal: Bool = false) {
        ...
        tokenValid = true
        keychainNeedsAccess = false
        tokenRecovery.authenticated()
    ...
    func checkToken(silent: Bool = false) async {
        ...
        tokenCheckEpoch = current
        checkingToken = true
        defer {
            if tokenCheckEpoch == current {
                tokenCheckEpoch = nil
                checkingToken = false
            }
        }
